...
<g><path style='opacity:1' fill='#fefffe' d='M 139.5,264.5 C 140.239,264.369 140.906,264.536 141.5,265C 139.85,268.94 139.017,273.107 139,277.5C 136.221,279.797 133.388,279.964 130.5,278C 131.286,275.04 132.452,272.206 134,269.5C 136.191,268.167 138.024,266.5 139.5,264.5 Z'/></g>
        ...
        /*
        stable var goals = [
                "#1 : Have a prototype and three different pilot communities to test on a low or null fee blockchain/Web3",
                "#2 : Have a full product (voting, services management, data aggregation) for promoting investment insights and three different mature communities to use as reference for newer communities",
                "#3 : Have extended features (NFT content rewards, social media / gaming partners to reflect the activity of each community on the metaverse) for promoting social cohesion / motivation to participate within financially autonomous communities and 50 new different community types to use as candidates for synergies",
                "#4 : Have extended management tools (automated and legally compliant accountability for local currency management, AI to detect synergies and export economic model) for growth acceleration of candidate mature communities for federative synergy structure / global currency and the recognition of public insitutions for them to operate and expand operations to infrastructure and banking capabilities"
        ];*/

        let members = HashMap.HashMap<Principal, Types.Member>(1, Principal.equal, Principal.hash);

        // Add Initial mentor for DAO
        let initialMentor : Types.Member = { 
        ...
        let webpageCanister = actor("jaamb-mqaaa-aaaaj-qa3ka-cai") : actor {
        ...
        public query func getStats() : async DAOStats {
                //let daoName : Text = await daoCanister.getName(); 
                //let daoGoals : [Text] = await daoCanister.getGoals();
                //let daoMembers : [Member] = await daoCanister.getMembers();

                return ({
                        name;
                        manifesto;
                        goals = Buffer.toArray(goals);//Buffer.fromArray(daoGoals);
        ...
        public shared ({ caller }) func graduate(student : Principal) : async Result<(), Text> {
                Result.assertOk( _verifyMemberRole(caller, #Mentor) );
                
                switch( members.get(student) ) {
                // Check if n is null
                case(null){ return #err("Member not found"); };
                case(? optFoundMember){
                        if(optFoundMember.role == #Student) {
                        ...
                        return #err("Member already has a graduate or higher role");
                };
                }
        };
        ...
        public shared ({ caller }) func createProposal(content : ProposalContent) : async Result<ProposalId, Text> {
                let resultVerifiedMember : Result<Member, Text> = _verifyMemberRole(caller, #Mentor);
        ...
        public shared ({ caller }) func voteProposal(proposalId : ProposalId, vote : Vote) : async Result<(), Text> {
                ...
                case (?member) {
                        ...
                        case (?proposal) {
                                ...
                                switch (newStatus) {
                                        case (#Accepted) {
                                                let resultExec : Result<(), Text> = _executeProposal(proposal.content);
        ...
        func _executeProposal(content : ProposalContent) : Result<(), Text> {
                ...
                case (#AddMentor(principal)) {
                        switch( members.get(principal) ) {
                        // Check if n is null
                        case(null){ return #err("Mentor candidate member not found - cannot execute proposal"); };
                        case(? optFoundMember){
                                if(optFoundMember.role == #Graduate) {
